...
pub async fn create_auto_push_tables(pool: &PgPool) -> Result<()> {
    ...
    // 检查是否已有默认配置
    let exists: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM auto_push_config WHERE id=1)")
            .fetch_one(pool)
            .await?;

    if !exists {
        sqlx::query("INSERT INTO auto_push_config (id, name) VALUES (1, '默认配置')")
            .execute(pool)
            .await?;
    }
...
// 更新唯一配置
pub async fn update_auto_push_config(
    pool: &PgPool,
    name: String,
    enabled: bool,
    window_minutes: i32,
...
pub async fn insert_push_log(
    pool: &PgPool,
    alert_type: i16,
    converged_id: uuid::Uuid,
) -> Result<()> {
    sqlx::query("INSERT INTO converged_push_logs (alert_type, converged_id) VALUES ($1, $2)")
        .bind(alert_type)
        .bind(converged_id)
        .execute(pool)
        .await?;
...
// 根据告警类型查询推送日志
pub async fn list_push_logs_by_type(
    pool: &PgPool,
    alert_type: i16,
    page: u64,
    page_size: u64,
    ...
    let row: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM converged_push_logs WHERE alert_type=$1")
            .bind(alert_type)
            .fetch_one(pool)
            .await?;
    let total = row.0 as u64;
