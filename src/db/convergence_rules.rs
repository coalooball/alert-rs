...
/// 创建收敛规则
pub async fn create_convergence_rule(pool: &PgPool, input: &ConvergenceRuleInput) -> Result<ConvergenceRuleRecord> {
...
/// 更新收敛规则
pub async fn update_convergence_rule(pool: &PgPool, id: Uuid, input: &ConvergenceRuleInput) -> Result<ConvergenceRuleRecord> {
    let record = sqlx::query_as::<_, ConvergenceRuleRecord>(
...
pub async fn get_convergence_rule_by_id(pool: &PgPool, id: Uuid) -> Result<ConvergenceRuleRecord> {
    let record = sqlx::query_as::<_, ConvergenceRuleRecord>(
        "SELECT * FROM convergence_rules WHERE id = $1"
    )
    .bind(id)
    .fetch_one(pool)
    .await?;
