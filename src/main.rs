...
async fn main() {
    ...
    if args.insert_mock_rules {
        ...
        match db::mock_rules::insert_all_mock_rules(&pool).await {
            Ok((convergence_count, correlation_count, filter_count, tag_count)) => {
                ...
                println!(
                    "   - 总计: {} 条",
                    convergence_count + correlation_count + filter_count + tag_count
                );
            }
            Err(e) => {
    ...
    let api_routes = Router::new()
        ...
        .route(
            "/api/network-attacks",
            get(api::alert_data::get_network_attacks),
        )
        .route(
            "/api/malicious-samples",
            get(api::alert_data::get_malicious_samples),
        )
        .route(
            "/api/host-behaviors",
            get(api::alert_data::get_host_behaviors),
        )
        .route(
            "/api/invalid-alerts",
            get(api::alert_data::get_invalid_alerts),
        )
        .route(
            "/api/threat-events",
            get(api::alert_data::get_threat_events),
        )
        .route(
            "/api/threat-events/:id",
            put(api::alert_data::update_threat_event),
        )
        // 原始告警查询路由
        .route(
            "/api/network-attacks/:id/raw",
            get(api::alert_data::get_raw_network_attacks_by_converged_id),
        )
        .route(
            "/api/malicious-samples/:id/raw",
            get(api::alert_data::get_raw_malicious_samples_by_converged_id),
        )
        .route(
            "/api/host-behaviors/:id/raw",
            get(api::alert_data::get_raw_host_behaviors_by_converged_id),
        )
        // 标签管理路由
        ...
        // 告警-标签关联路由
        .route(
            "/api/alerts/:alert_type/:alert_id/tags",
            get(api::alert_tag::get_alert_tags),
        )
        .route(
            "/api/alerts/:alert_type/:alert_id/tags",
            post(api::alert_tag::add_alert_tag),
        )
        .route(
            "/api/alerts/:alert_type/:alert_id/tags/batch",
            post(api::alert_tag::batch_add_alert_tags),
        )
        .route(
            "/api/alerts/:alert_type/:alert_id/tags/:tag_id",
            delete(api::alert_tag::remove_alert_tag),
        )
        .route(
            "/api/alerts/:alert_type/:alert_id/tags",
            delete(api::alert_tag::remove_all_alert_tags),
        )
        .route(
            "/api/tags/:tag_id/alerts",
            get(api::alert_tag::get_alerts_by_tag),
        )
        // 字段定义路由
        .route(
            "/api/alert-fields",
            get(api::alert_fields::get_alert_fields),
        )
        .route(
            "/api/alert-fields/groups",
            get(api::alert_fields::get_common_field_groups),
        )
        // DSL 编译路由
        .route(
            "/api/rules/convergence/compile",
            post(api::dsl_compile::compile_converge_rule),
        )
        .route(
            "/api/rules/correlation/compile",
            post(api::dsl_compile::compile_correlate_rule),
        )
        // 收敛规则路由
        .route(
            "/api/rules/convergence",
            get(api::rules::get_convergence_rules),
        )
        .route(
            "/api/rules/convergence",
            post(api::rules::create_convergence_rule),
        )
        .route(
            "/api/rules/convergence/:id",
            get(api::rules::get_convergence_rule_by_id),
        )
        .route(
            "/api/rules/convergence/:id",
            put(api::rules::update_convergence_rule),
        )
        .route(
            "/api/rules/convergence/:id",
            delete(api::rules::delete_convergence_rule),
        )
        // 关联规则路由
        .route(
            "/api/rules/correlation",
            get(api::rules::get_correlation_rules),
        )
        .route(
            "/api/rules/correlation",
            post(api::rules::create_correlation_rule),
        )
        .route(
            "/api/rules/correlation/:id",
            get(api::rules::get_correlation_rule_by_id),
        )
        .route(
            "/api/rules/correlation/:id",
            put(api::rules::update_correlation_rule),
        )
        .route(
            "/api/rules/correlation/:id",
            delete(api::rules::delete_correlation_rule),
        )
        // 过滤规则路由
        ...
        .route(
            "/api/rules/filter/:id",
            get(api::rules::get_filter_rule_by_id),
        )
        .route("/api/rules/filter/:id", put(api::rules::update_filter_rule))
        .route(
            "/api/rules/filter/:id",
            delete(api::rules::delete_filter_rule),
        )
        // 标签规则路由
        ...
        // 自动推送配置路由 (单例)
        .route(
            "/api/auto/push-config",
            get(api::auto_publish::get_push_config),
        )
        .route(
            "/api/auto/push-config",
            put(api::auto_publish::update_push_config),
        )
        // 推送日志查询路由
        .route("/api/auto/push-logs", get(api::auto_publish::get_push_logs))
        // 手动触发推送（用于测试）
        .route(
            "/api/auto/publish-converged",
            post(api::auto_publish::publish_converged_by_window),
        )
...
// 获取告警类型枚举
async fn get_alarm_types(State(state): State<Arc<AppState>>) -> Json<AlarmTypesConfig> {
...
async fn spa_fallback(_uri: Uri) -> Response {
    ...
    match tokio::fs::read_to_string("frontend/dist/index.html").await {
        ...
        Err(_) => (StatusCode::NOT_FOUND, "Frontend files not found").into_response(),
    }
}
