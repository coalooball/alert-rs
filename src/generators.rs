use crate::models::{NetworkAttackAlert, MaliciousSampleAlert, HostBehaviorAlert};
use chrono::Utc;
use rand::Rng;
...
pub fn generate_network_attack_alert() -> NetworkAttackAlert {
    ...
    let id = format!("NA-2024-{:03}-{:06X}", rng.gen_range(1..999), rng.gen::<u32>() & 0xFFFFFF);
    
    let src_ips = ["192.168.1.100", "10.0.1.50", "172.16.0.10", "192.168.2.200"];
    let dst_ips = ["185.234.218.100", "45.67.89.123", "203.0.113.50", "198.51.100.20"];
    ...
    NetworkAttackAlert {
        ...
        control_rule_id: format!("RULE-{}-2024-{:03}", if apt.is_empty() { "SEC" } else { "APT" }, rng.gen_range(1..999)),
        ...
        session_id: format!("SESSION-{}-{:06}", Utc::now().format("%Y%m%d"), rng.gen::<u32>() % 999999),
        ip_version: 4,
        ...
        source_file_path: format!("/data/traffic/2024/12/25/capture_{}.pcap", timestamp % 999999),
        signature_id: format!("SIG-{}-{:03}", if apt.is_empty() { "ATK" } else { "APT" }, rng.gen_range(1..999)),
        attack_payload: format!(r#"{{"method":"GET","uri":"/api/data?id={}"}}"#, rng.gen::<u32>()),
...
pub fn generate_malicious_sample_alert() -> MaliciousSampleAlert {
    let mut rng = rand::thread_rng();
    let sample_types = [
        ("Emotet银行木马变种检测", "检测到Emotet银行木马最新变种，该样本具有窃取银行凭证和传播能力", "Emotet", 2003, "Trojan"),
        ("WannaCry勒索软件检测", "发现WannaCry勒索软件样本，该样本会加密系统文件并索要比特币赎金", "WannaCry", 2005, "Ransomware"),
        ("Mirai僵尸网络样本", "检测到Mirai僵尸网络恶意样本，可能用于DDoS攻击", "Mirai", 2004, "Botnet"),
        ("挖矿木马XMRig变种", "发现门罗币挖矿木马XMRig变种，会消耗大量系统资源", "XMRig", 2006, "Miner"),
        ("Cobalt Strike后门", "检测到Cobalt Strike木马样本，常用于APT攻击", "CobaltStrike", 2003, "Backdoor"),
    ];
    
    let (name, desc, family, subtype, type_name) = sample_types[rng.gen_range(0..sample_types.len())];
    let severity = rng.gen_range(2..=3);
    let timestamp = Utc::now().timestamp_millis();
    let id = format!("MS-2024-{:03}-{:06X}", rng.gen_range(1..999), rng.gen::<u32>() & 0xFFFFFF);
    ...
    MaliciousSampleAlert {
        ...
        apt_group: if family == "CobaltStrike" { "APT29".to_string() } else { String::new() },
...
pub fn generate_host_behavior_alert() -> HostBehaviorAlert {
    let mut rng = rand::thread_rng();
    let behavior_types = [
        ("XMRig挖矿进程检测", "检测到主机运行XMRig挖矿程序，占用大量CPU资源进行门罗币挖矿", 3001, "/tmp/.system/xmrig", "挖矿"),
        ("勒索软件文件加密行为", "检测到大量文件被加密并添加.locked扩展名，疑似勒索软件攻击", 3002, "C:\\Users\\admin\\AppData\\Roaming\\svchost.exe", "加密"),
        ("远程桌面暴力破解", "检测到针对RDP服务的大量失败登录尝试", 3004, "", "爆破"),
        ("敏感数据外传", "检测到大量敏感文件被上传到外部服务器", 3008, "/usr/bin/curl", "窃取"),
        ("横向移动攻击", "检测到使用WMI进行横向移动的可疑行为", 3007, "C:\\Windows\\System32\\wbem\\wmic.exe", "移动"),
    ];
    
    let (name, desc, subtype, process_path, attack_type) = behavior_types[rng.gen_range(0..behavior_types.len())];
    let severity = rng.gen_range(2..=3);
    let timestamp = Utc::now().timestamp_millis();
    let id = format!("HB-2024-{:03}-{:06X}", rng.gen_range(1..999), rng.gen::<u32>() & 0xFFFFFF);
    
    let hostnames = ["DB-SERVER-01", "WEB-SERVER-02", "FIN-WORKSTATION-10", "DEV-PC-05"];
    let ips = ["192.168.10.50", "10.0.2.100", "172.16.5.20", "192.168.2.110"];
    ...
    HostBehaviorAlert {
        ...
        control_rule_id: format!("RULE-{}-2024-{:03}", attack_type.to_uppercase(), rng.gen_range(1..999)),
        ...
        src_ip: String::new(),
        src_port: None,
        dst_ip: if subtype == 3001 { "pool.minexmr.com".to_string() } else { String::new() },
        dst_port: if subtype == 3001 { Some(4444) } else { None },
        protocol: if subtype == 3001 { "TCP".to_string() } else { String::new() },
        ...
        user_account: if subtype == 3001 { "www-data".to_string() } else { "admin".to_string() },
        terminal_os: if process_path.starts_with('/') { "Ubuntu 20.04.3 LTS".to_string() } else { "Windows 10 Pro".to_string() },
        ...
        src_process_path: if process_path.starts_with('/') { "/usr/sbin/apache2".to_string() } else { "C:\\Windows\\System32\\explorer.exe".to_string() },
        src_process_cli: String::new(),
        register_key_name: if !process_path.starts_with('/') { "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run".to_string() } else { String::new() },
        register_key_value: if !process_path.starts_with('/') { process_path.to_string() } else { String::new() },
        register_path: if !process_path.starts_with('/') { "HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run".to_string() } else { String::new() },
