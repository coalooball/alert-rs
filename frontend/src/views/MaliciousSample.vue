            ...
            <el-collapse v-model="activeRawAlert" accordion v-if="rawAlerts.length > 0">
              ...
              >
                <el-descriptions :column="2" border size="small">
                  ...
                  <el-descriptions-item label="样本名称">{{ rawAlert.sample_original_name }}</el-descriptions-item>
                  <el-descriptions-item label="文件类型">{{ rawAlert.file_type }}</el-descriptions-item>
                  <el-descriptions-item label="样本家族">{{ rawAlert.sample_family }}</el-descriptions-item>
                  <el-descriptions-item label="MD5">{{ rawAlert.md5 }}</el-descriptions-item>
...
import { getMaliciousSamples, getAlarmTypes, getRawMaliciousSamplesByConvergedId, getAllTags, getAlertTags, addAlertTag, removeAlertTag } from '../api'
...
const loadTagsForAlerts = async () => {
  for (const alert of tableData.value) {
    try {
      const response = await getAlertTags('malicious_sample', alert.id)
...
const loadRawAlerts = async (convergedAlertId) => {
  ...
  try {
    const response = await getRawMaliciousSamplesByConvergedId(convergedAlertId)
...
const getAlarmSubtypeName = (code) => {
  ...
  return alarmTypes.value.malicious_sample?.subtypes?.[key] || code
}
...
const loadCurrentAlertTags = async () => {
  ...
  try {
    const response = await getAlertTags('malicious_sample', currentRow.value.id)
...
const handleAddTag = async (tagId) => {
  try {
    await addAlertTag('malicious_sample', currentRow.value.id, tagId)
...
const handleRemoveTag = async (tagId) => {
  try {
    await removeAlertTag('malicious_sample', currentRow.value.id, tagId)
